...
# pp coords
pp min_x = coords.min_by {|e| e.x}.x
pp max_x = coords.max_by {|e| e.x}.x
pp min_y = coords.min_by {|e| e.y}.y
pp max_y = coords.max_by {|e| e.y}.y
...
grid.each_with_index do |col, y|
  col.each_with_index do |row, x|
    ...
    grid[y][x] = min_distances if min_distances.last.length == 1
  end
end
